...
# Kobixel — extensão para Aseprite

<img src="./kobixel.jpg" alt="Kobixel" width="600px"></img>

**Kobixel** vem de *kobo* (工房), "oficina"/"ateliê" em japonês, mais
*pixel* — uma oficina para o seu pixel art, em forma de extensão do
Aseprite.
...
`kobixel-X.Y.Z.aseprite-extension`, instalado dentro do Aseprite. Nada
...
volta. `tools/kobixel-gemini-web/` (abaixo) é o comando padrão hoje e é o único
...
| Opção | Resultado |
|---|---|
| `tools/kobixel-gemini-web/` (Playwright + Chrome já logado) | **Recomendado.** Sem chave de API, usa a assinatura Gemini Pro. ~20-40s por edição. Ver setup abaixo. |
...
- **Aseprite 1.3+** — o script usa `Image:pixels()`, `app.fs` e
  `app.transaction`.
- **Node.js 18+** e npm — pra instalar e rodar o backend recomendado
  `tools/kobixel-gemini-web` (Playwright).
- **Google Chrome** instalado — o backend recomendado se conecta numa
  janela real do Chrome pela porta de debug; Chromium ou outro navegador
  não vai ter o seu login do Google já feito.
- **Uma conta Google com geração de imagem no Gemini.** O Nano Banana
  funciona na camada gratuita também (com uma cota diária baixa, que o
...
1. Gere o `.aseprite-extension` com o script de release:
   ...
   builds antigos, e gera `kobixel-X.Y.Z.aseprite-extension` na raiz do
...
3. Reinicie o Aseprite. O comando aparece em `Edit > Kobixel...`.
...
Depois de editar `kobixel.lua`, rode:
...
processo em si). `tools/kobixel-gemini-web/edit.mjs` imprime marcadores de
...
- Log de cada execução: `<temp>/aseprite-kobixel/kobixel-<stamp>.log` (um
